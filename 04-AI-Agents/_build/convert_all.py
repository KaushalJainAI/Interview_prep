...
TX = Path(__file__).resolve().parent.parent
OUT = TX/"out"; OUT.mkdir(exist_ok=True); HTML = OUT/"_html"; HTML.mkdir(exist_ok=True)
DSA_BUILD = TX.parent / "01-CS-Fundamentals" / "dsa-patterns" / "_build"
sys.path.insert(0, str(DSA_BUILD))
from md_to_docx import convert as md_to_docx
shutil.copytree(TX/"diagrams", OUT/"diagrams", dirs_exist_ok=True)
shutil.copytree(TX/"diagrams", HTML/"diagrams", dirs_exist_ok=True)
...
def to_pdf(md, pdf):
    ...
    subprocess.run([EDGE,"--headless=new","--disable-gpu",f"--print-to-pdf={pdf}","--no-pdf-header-footer",html.resolve().as_uri()], check=True, timeout=90)
for md in sorted(TX.glob("*.md")):
    docx = OUT/f"{md.stem}.docx"; pdf = OUT/f"{md.stem}.pdf"
    print(">>", md.name)
    md_to_docx(md, docx, md.parent); to_pdf(md, pdf)
print("done.")
