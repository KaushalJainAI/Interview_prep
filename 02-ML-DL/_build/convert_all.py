"""Convert ML/DL markdown to .docx and .pdf using shared converters."""
...
# Import md_to_docx from the DSA build folder
DSA_BUILD = ML.parent / "01-CS-Fundamentals" / "dsa-patterns" / "_build"
sys.path.insert(0, str(DSA_BUILD))
from md_to_docx import convert as md_to_docx

# Stage diagrams next to outputs
shutil.copytree(ML / "diagrams", OUT / "diagrams", dirs_exist_ok=True)
shutil.copytree(ML / "diagrams", HTML / "diagrams", dirs_exist_ok=True)
...
def md_to_pdf(md_path: Path, pdf_path: Path):
    html_path = HTML / (md_path.stem + ".html")
    body = md_lib.markdown(md_path.read_text(encoding="utf-8"), extensions=["fenced_code", "tables", "codehilite"])
    ...
    subprocess.run([EDGE, "--headless=new", "--disable-gpu",
                    ...
                    html_path.resolve().as_uri()], check=True, timeout=90)

for md in sorted(ML.glob("*.md")):
